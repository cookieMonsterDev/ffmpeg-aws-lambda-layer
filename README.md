...
## Will this be competitive with the hudd-cloud setup?

`hudd-cloud/lambdas/attachments` currently uses FFmpeg through a Lambda
layer at the non-standard path `/opt/lambda-layer-structure/bin/ffmpeg`,
with overrides via the `FFMPEG_BINARY_PATH` / `FFPROBE_BINARY_PATH` env
vars and a final `PATH` fallback. Its README recommends the legacy
`ffmpeg-lambda-layer` SAR app
(`arn:aws:serverlessrepo:us-east-1:145266761615:applications~ffmpeg-lambda-layer`),
which ships **FFmpeg 4.1.3** built for **Amazon Linux 2** and targets
`nodejs10.x` / `python3.6` / `ruby2.5` / `java8` / `go1.x` runtimes.

Side-by-side:

| Property                    | Public `ffmpeg-lambda-layer` (used today)              | This layer (`ffmpeg-lambda-layer-for-amazon-linux-2023`)                  |
| --------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------- |
| FFmpeg version              | 4.1.3 (2019)                                           | 7.0.2 (auto-rolls forward)                                                |
| Built for                   | Amazon Linux 2                                         | Amazon Linux 2023                                                         |
| Listed `CompatibleRuntimes` | `nodejs10.x`, `python3.6`, `ruby2.5`, `java8`, `go1.x` | `nodejs18/20/22.x`, `python3.11/12/13`, `provided.al2`, `provided.al2023` |
| Layer install path          | `/opt/bin/ffmpeg`                                      | `/opt/bin/ffmpeg`                                                         |
| License                     | GPL-2.0-or-later                                       | GPL-3.0-or-later (johnvansickle release tarball)                          |

**Conclusion: yes, this is the strictly better option for hudd-cloud.**
The current setup is functional only because the old layer's binary
happens to still run on newer Lambda runtimes — but its
`CompatibleRuntimes` no longer match what hudd-cloud uses (`provided.al2`
Go custom runtime), and AWS has been progressively deprecating those
older runtimes.

### How hudd-cloud should consume this layer

The Go handler already does the right thing — it tries
`FFMPEG_BINARY_PATH` env, then a hard-coded layer path, then `PATH`. Two
options to switch:

**Option A — set env vars (no code change):**

```yaml
# hudd-cloud's attachments function (SAM/CloudFormation)
Environment:
  Variables:
    FFMPEG_BINARY_PATH: /opt/bin/ffmpeg
    FFPROBE_BINARY_PATH: /opt/bin/ffprobe
Layers:
  - !GetAtt Ffmpeg.Outputs.LayerVersionArn
```

**Option B — drop the env vars and rely on the `PATH` fallback.**
`/opt/bin` is already on `PATH` in every Lambda runtime, so calls to
`ffmpeg` / `ffprobe` will resolve correctly even without explicit
binary paths. The `os.Stat` check in `handler.go:950` falls through to
the bare-name `ffmpeg` / `ffprobe` when its default
`/opt/lambda-layer-structure/bin/...` paths don't exist — which is
exactly what will happen with this layer.

Either option works without changing `handler.go`.
